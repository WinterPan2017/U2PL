...
from u2pl.utils.mask import RandomMaskGenerator
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 1):
            ...
            image_all = torch.cat((image_l, image_u_aug, image_masked))
            ...
            pred_m = outs["pred"][num_labeled+num_unlabeled:]
            ...
            pred_m_large = F.interpolate(
                pred_m, size=(h, w), mode="bilinear", align_corners=True
            )
            ...
            model_teacher.train()
            with torch.no_grad():
                out_t = model_teacher(torch.cat((image_l, image_u_aug)))
        ...
        loss = sup_loss + unsup_loss + contra_loss + lcr_loss
        ...
        if i_iter % 10 == 0 and rank == 0:
            logger.info(
                ...
                "Reliable Ratio {reliable_ratio_lcr_meter.val:.3f} ({reliable_ratio_lcr_meter.avg:.3f})\t"
                "LR {lr.val:.5f}".format(
                    cfg["dataset"]["n_sup"],
                    contra_flag,
                    i_iter,
            ...
            tb_logger.add_scalar("Reliable Ratio", reliable_ratio_lcr_meter.val, i_iter)
